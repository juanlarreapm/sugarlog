...
function App() {
  ...
  return (
    <div style={{
      ...
      {modal === 'bg' && (
        <ModalShell theme={theme} title="Blood sugar" onClose={() => setModal(null)} maxW={maxW}>
          <BgForm theme={theme} onSave={addEntry} onCancel={() => setModal(null)}/>
        </ModalShell>
      )}
      {modal === 'insulin' && (
        <ModalShell theme={theme} title="Insulin" onClose={() => setModal(null)} maxW={maxW}>
      ...
      {modal === 'drift' && (
        <ModalShell theme={theme} title="CGM drift" onClose={() => setModal(null)} maxW={maxW}>
      ...
      {!modal && <TabBar tab={tab} setTab={setTab} theme={theme} maxW={maxW}/>}
    </div>
  );
}
...
function ModalShell({ theme, title, onClose, maxW, children }) {
  return (
    <div style={{
      position: 'fixed', inset: 0, background: theme.bg,
      display: 'flex', flexDirection: 'column', zIndex: 30,
    }}>
      <div style={{ width: '100%', maxWidth: maxW || '100%', margin: '0 auto', flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
        <div style={{
          display: 'flex', alignItems: 'center', justifyContent: 'space-between',
...
function TabBar({ tab, setTab, theme, maxW }) {
  const tabs = [
  ...
  return (
    <div className="tabbar-wrap" style={{
